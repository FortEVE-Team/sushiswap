...
describe("MasterChefV2", function () {
  before(async function () {
    await prepare(this, ["MasterChef", "SerabeToken", "ERC20Mock", "MasterChefV2", "RewarderMock", "RewarderBrokenMock"])
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.MasterChef, [this.serabe.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.MasterChefV2, [this.chef.address, this.serabe.address, 1]],
  ...
  describe("PendingSerabe", function () {
    it("PendingSerabe should equal ExpectedSerabe", async function () {
      ...
      await advanceBlock()
      let expectedSerabe = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingSerabe = await this.chef2.pendingSerabe(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accSerabePerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SERABE and reward", async function () {
      ...
      let expectedSerabe = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedSerabe)
