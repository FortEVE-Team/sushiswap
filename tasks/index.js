...
const { MINICHEF_ADDRESS } = require("@serabeswap/sdk")
...
task("migrate", "Migrates liquidity from Uniswap to SerabeSwap")
...
task("bar:enter", "SerabeBar enter")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  await run("erc20:approve", { token: serabe.address, spender: bar.address })
  
  await (await bar.connect(await getNamedSigner("dev")).enter(amount)).wait()
});

task("bar:leave", "SerabeBar leave")
.addParam("amount", "Amount")
.setAction(async function ({ amount }, { ethers: { getNamedSigner } }, runSuper) {
  const serabe = await ethers.getContract("SerabeToken")
...
task("maker:serve", "SerabeBar serve")
.addParam("a", "Token A")
.addParam("b", "Token B")
