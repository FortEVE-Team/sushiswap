...
// SerabeMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Serabe from pretty much anything!
// This contract handles "serving up" rewards for xSerabe holders by trading tokens collected from Kashi fees for Serabe.
contract SerabeMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 serabeOut) {
        if (token0 == serabe) {
            IERC20(token0).safeTransfer(bar, amount0);
            serabeOut = amount0;
        } else if (token0 == weth) {
            serabeOut = _swap(token0, serabe, amount0, bar);
        } else {
            ...
            serabeOut = _convertStep(bridge, amountOut);
        }
    }
